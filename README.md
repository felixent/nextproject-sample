...
```sh
#into your `"nextproject-sample"` folder directory.
$ cd /{your_directory}/nextproject-sample
...
```sh
$ pm2 start npm --name "nextproject-sample -- start
```

destroy process:
```sh
$ pm2 stop "nextproject-sample" & pm2 delete "nextproject-sample"
```
...
$ pm2 start /{your_directory}/nextproject-sample/ecosystem.config.js --restart-delay=3000
$ pm2 save
```
...
$ rm -rf /{your_directory}/nextproject-sample/.next  # Delete the. Next folder
$ npm run build
$ pm2 start ecosystem.config.js
```
...
```sh
$ cd /Applications/MAMP/htdocs/nextproject-sample
$ touch Dockerfile
$ vi Dockerfile
```
...
```sh
$ docker build -t nextproject-sample:v1 .
```
...
```sh
$ docker run -p 3000:3000 nextproject-sample:v1
```
...
```sh
$ docker run -p 3000:3000 -d nextproject-sample:v1
```
...
 
```bash
$ docker tag nextproject-sample:v1 192.168.1.140:5000/nextproject-sample-v1
```
push it

```bash
$ docker push 192.168.1.140:5000/nextproject-sample-v1
```
 
...
```bash
$ docker pull 192.168.1.140:5000/nextproject-sample-v1
```
...
```sh
$ docker commit 16cb27979742 nextproject-sample:v1
```
...
```sh
$ docker save nextproject-sample:v1 -o ./nextproject-sample-v1.tar
```
...
```sh
$ docker build --platform linux/arm64 -t nextproject-sample-arm64:v1 .
```
...
$ docker buildx build --platform linux/arm64 -t nextproject-sample-arm64:v1 .
```
